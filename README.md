...
> **Η Ολοκληρωμένη Επαγγελματική Πλατφόρμα για Σύγχρονες Κουζίνες**

Το **ChefSuite** είναι μια cloud-based, AI-powered εφαρμογή διαχείρισης κουζίνας που σχεδιάστηκε αποκλειστικά για επαγγελματίες σεφ, F&B managers και ομάδες εστιατορίων. Συνδυάζει σε ένα ενιαίο περιβάλλον τη διαχείριση συνταγών και μενού, τον έλεγχο αποθέματος, τον οικονομικό σχεδιασμό, τις λειτουργίες κουζίνας, τη διαχείριση προσωπικού και τα εργαλεία επικοινωνίας — αντικαθιστώντας δεκάδες ξεχωριστά εργαλεία με μία ολοκληρωμένη λύση που λειτουργεί από οποιαδήποτε συσκευή, ακόμα και offline.
...
- **Σάρωση Συνταγής** από φωτογραφία ή χειρόγραφο κείμενο (OCR)
- **Import από Excel/CSV** με AI-powered αυτόματη αντιστοίχιση στηλών, preview και bulk επεξεργασία πριν την εισαγωγή
...
- **Variations**: σύνδεση συνταγών ως παραλλαγές βάσης (π.χ. vegetarian/gluten-free εκδόσεις)
...
- **Menu Engineering** (Stars / Plowhorses / Puzzles / Dogs): τετραδιαστατική ανάλυση δημοτικότητας και κερδοφορίας
- **Φύλλο Παραγωγής** (Production Sheet) οργανωμένο ανά workstation με κατάσταση εκτέλεσης
- **Λίστα Αγορών** (Shopping List) αυτόματα δημιουργημένη από το μενού
...
- **DishInfo QR**: σελίδα πληροφοριών πιάτου σε **9 γλώσσες** (Ελληνικά, Αγγλικά, Βουλγαρικά, Ουκρανικά, Ρουμανικά, Σερβικά, Σλοβάκικα, Πολωνικά, Τσέχικα) για πολυεθνικές ομάδες ή πελάτες
...
- **QR Code ανά Item** αποθήκης για γρήγορη αναζήτηση και ιστορικό κινήσεων
- **Κινήσεις Αποθέματος**: πλήρες ιστορικό εισαγωγών, εξαγωγών και κατανάλωσης
- **Πρόβλεψη Αποθέματος**: αυτόματος υπολογισμός ημερών-μέχρι-εξάντλησης βάσει κατανάλωσης 30 ημερών
- **Stocktake**: περιοδική καταμέτρηση αποθέματος με σύγκριση έναντι συστήματος
- **Προμηθευτές**: αρχείο προμηθευτών με σύνδεση σε inventory items
- **Purchase Orders**: δημιουργία παραγγελιών αγοράς, **AI parsing τιμολογίων**, παρακολούθηση κατάστασης και αυτόματη ενημέρωση αποθέματος κατά παραλαβή
- **Παρακολούθηση Τιμών** (Price Tracking) με αυτόματη ειδοποίηση για αυξήσεις >10% ανά item
...
- **Prep από Μενού**: αυτόματη δημιουργία prep tasks από τις ανάγκες ενός συγκεκριμένου μενού
- **HACCP Log**: καταγραφή ελέγχων θερμοκρασίας ανά σημείο (CCP), αυτόματοί χρονοδιακόπτες υπενθύμισης (1–24h), εκτύπωση κενής φόρμας HACCP
- **Waste Log**: καταγραφή απωλειών τροφίμων ανά κατηγορία με μηνιαία στατιστικά και ανάλυση κόστους
...
- **Ρολόι Παρουσίας** (Timeclock): clock in/out με αυτόματο υπολογισμό ωρών εργασίας ανά ημέρα
...
- **Γαστρονομικό Γλωσσάρι** (Culinary Glossary): ορολογία κουζίνας σε ελληνικά και αγγλικά
- **Chef Journal**: προσωπικές σημειώσεις σεφ με ετικέτες (tags) και mood tracking

---

## Τεχνολογικό Stack (Tech Stack)

| Επίπεδο | Τεχνολογία |
|---|---|
| **Frontend Framework** | React 18 + TypeScript 5.6 |
| **Build Tool** | Vite 5 |
| **UI / Styling** | Tailwind CSS 3 + custom design system |
| **Routing** | React Router v7 |
| **Internationalization** | i18next + react-i18next (EL / EN) |
| **Icons** | Lucide React |
...
All migrations are in `supabase/migrations/`. If you prefer, run the SQL files in order through the Supabase SQL editor.
...
│   ├── prep/         # PrepTaskForm, PrepTemplatesDrawer, PrepFromMenuDrawer
...
Για αυτόματη αποστολή αναφοράς κάθε Δευτέρα στις 08:00, προσθέστε το παρακάτω cron job μέσω Supabase Dashboard → Database → Extensions → pg_cron:
...
*ChefSuite — Built for professional kitchens.*
