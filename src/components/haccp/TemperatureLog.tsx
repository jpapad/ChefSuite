    ...
    <div className={cn(
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-white/40 mt-0.5">
          {t('tempLog.acceptable')} {appliance.min_temp}°C – {appliance.max_temp}°C
        </p>
        {!pass && log.corrective_action && (
          <div className="mt-2 rounded-lg border border-red-500/20 bg-red-500/10 px-3 py-2 text-xs text-red-300">
            <span className="font-semibold">{t('tempLog.corrective')}</span> {log.corrective_action}
          </div>
        )}
      </div>
      <button
...
function ApplianceCard({ appliance, shift, onLog }: ApplianceCardProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (outRange && !form.corrective.trim()) {
      setForm((f) => ({ ...f, error: t('tempLog.requiredField') }))
      return
    }
    ...
    try {
      ...
      setForm((f) => ({ ...f, saving: false, error: err instanceof Error ? err.message : t('common.saveFailed') }))
    }
  }

  const shiftLabel = shift === 'morning' ? t('tempLog.morning') : t('tempLog.evening')
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-white/40">
            {appliance.type === 'freezer' ? t('tempLog.freezer') : t('tempLog.fridge')} · {appliance.min_temp}°C – {appliance.max_temp}°C
          </p>
        </div>
        <span className="shrink-0 text-xs text-white/30">{shiftLabel} {t('tempLog.shift')}</span>
      </div>

      {/* Temperature input */}
      <div>
        <label className="mb-1.5 block text-xs font-medium text-white/60">{t('tempLog.temperature')}</label>
        <div className="relative">
          ...
          <input
            type="number"
            step="0.1"
            placeholder={t('tempLog.tempPlaceholder')}
            value={form.temp}
      ...
      {outRange && (
        <div className="rounded-xl border border-red-500/30 bg-red-500/5 p-3 space-y-2">
          <div className="flex items-center gap-2 text-sm font-semibold text-red-400">
            ...
            {t('tempLog.outOfRangeLabel')}
          </div>
          <textarea
            placeholder={t('tempLog.correctivePlaceholder')}
...
export function TemperatureLog({ appliances, logs, shift, onLog, onDeleteLog, loading }: TemperatureLogProps) {
  ...
  if (loading) {
    return <div className="py-12 text-center text-white/40 text-sm">{t('common.loading')}</div>
  }
  ...
  if (appliances.length === 0) {
    return (
      <div className="py-12 flex flex-col items-center gap-3 text-center">
        ...
        <p className="text-white/50 text-sm">{t('tempLog.noDevices')}</p>
        <p className="text-white/30 text-xs">{t('tempLog.noDevicesHint')}</p>
      </div>
    )
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <span className="shrink-0 text-xs font-semibold text-white/50">
          ...
          {failCount > 0 && <span className="text-red-400 ml-1.5">• {failCount} {t('tempLog.outOfRange')}</span>}
        </span>
      </div>
